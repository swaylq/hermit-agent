...
3. **Invocation flags.** Pass `--no-session-persistence` (the `-p` companion) so the post-task JSONL flush doesn't drag past the timeout — a substantial outreach turn produces hundreds of KB of session-log writes during exit, and that disk flush has been observed to hang `claude -p` long enough to trip exit 124 even after the prompt's work was already complete (Telegram report sent). Do **not** pass `--bare` on agents authenticated through Claude Max OAuth (the macOS Keychain path): `--bare` skips keychain reads and demands `ANTHROPIC_API_KEY`/`apiKeyHelper`, so an OAuth agent fires once and fails with `Not logged in · Please run /login`.
